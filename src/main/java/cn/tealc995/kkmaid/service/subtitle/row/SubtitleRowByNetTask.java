...
/**
 * @program: KK-Maid
 * @description: 读取网络中的字幕文件源文件
 * @author: Leck
 * @create: 2025-02-03 21:05
 */
public class SubtitleRowByNetTask extends SubtitleRowBaseTask {
    private static final Logger LOG = LoggerFactory.getLogger(SubtitleRowByNetTask.class);
    private final String httpUrl;
    private Charset charset = StandardCharsets.UTF_8;

    public SubtitleRowByNetTask(String httpUrl) {
        this.httpUrl = httpUrl;
    }

    public SubtitleRowByNetTask(String httpUrl, Charset charset) {
        this.httpUrl = httpUrl;
        this.charset = charset;
    }

    @Override
    protected ResponseBody<String> call() {
        try {
            if (SupportSubtitleFormat.getType(httpUrl) == SupportSubtitleFormat.LRC) {
                String content = download();
                return ResponseBody.create(200, "成功下载字幕文件", content);
